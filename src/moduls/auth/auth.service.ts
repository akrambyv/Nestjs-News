...
export class AuthService {
    ...
    async signIn(params: AuthSignInDto) {
        let user = await this.userRepo.findOne({ where: { username: params.username } });
        ...
        if (!checkPassword) throw new UnauthorizedException("User or password is wrong");
    ...
    async register(params: AuthRegisterDto) {
        let checkUsername = await this.userRepo.exists({ where: { username: params.username } });
